...
import { applyOrderSyncResult, buildOpenPositionFromEntrySignal, markPositionClosing, reconcilePositionToTokenBalance } from "../dist/src/position-state.js";
...
run("token balance reconciliation closes a stale position when shares are gone", () => {
  const signal = makeEntrySignal();
  const basePosition = buildOpenPositionFromEntrySignal(signal, {
    filled_shares: 20,
    filled_notional: 8.2,
...
run("token balance reconciliation clamps open shares to the live token balance", () => {
  const signal = makeEntrySignal();
  const basePosition = buildOpenPositionFromEntrySignal(signal, {
    filled_shares: 20,
    filled_notional: 8.2,
